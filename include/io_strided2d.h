...
ops_dat create_strided_ops_dat(ops_dat dat, size_t stride_i, size_t stride_j, ) {
  ...
  int offset_new[] = {(int)abs(new_dat->d_m[0]), (int)abs(new_dat->d_m[1])};
  ...
  for (int j = offset[1]; j < size[1] - dat->d_p[1]; j += stride_j) {
    for (int i = offset[0]; i < size[0] - dat->d_p[0]; i += stride_i) {
      ...
      const size_t index_new = index3d((i - offset[0]) / stride_i + offset_new[0],
                                       (j - offset[1]) / stride_j + offset_new[1], new_dat->size) *
                               new_dat->elem_size;
