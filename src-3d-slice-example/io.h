...
void ops_write_plane_group_strided(char *slice_name0, ops_dat rho_B0, ops_dat rhou0_B0, ops_dat rhou1_B0,
  ...
  double cpu_stride_start, elapsed_stride_start;
  double cpu_stride_end, elapsed_stride_end;
  ops_timers(&cpu_stride_start, &elapsed_stride_start);
  ops_dat rho_B0_strided =         create_strided_ops_dat_without_halo_cells(rho_B0, stride_i, stride_j, stride_k);
  ops_dat rhou0_B0_strided =       create_strided_ops_dat_without_halo_cells(rhou0_B0, stride_i, stride_j, stride_k);
  ops_dat rhou1_B0_strided =       create_strided_ops_dat_without_halo_cells(rhou1_B0, stride_i, stride_j, stride_k);
  ops_dat rhou2_B0_strided =       create_strided_ops_dat_without_halo_cells(rhou2_B0, stride_i, stride_j, stride_k);
  ops_dat rhoE_B0_strided =        create_strided_ops_dat_without_halo_cells(rhoE_B0, stride_i, stride_j, stride_k);
  ops_dat WENO_filter_B0_strided = create_strided_ops_dat_without_halo_cells(WENO_filter_B0, stride_i, stride_j, stride_k);
  ops_timers(&cpu_stride_end, &elapsed_stride_end);
  ops_printf("-----------------------------------------\n");
  ops_printf("Time to write create strided data: %lf\n", elapsed_stride_end - elapsed_stride_start);
  ops_printf("-----------------------------------------\n");

  ops_write_plane_group_hdf5({{2, (block0np2 / stride_k) / 2}}, slice_name0,
                             {{rho_B0_strided, rhou0_B0_strided, rhou1_B0_strided, rhou2_B0_strided, rhoE_B0_strided,
                               WENO_filter_B0_strided}});

  ops_timers(&cpu_end0, &elapsed_end0);
  ops_printf("-----------------------------------------\n");
  ops_printf("Time to write strided and sliced HDF5 file: %lf\n", elapsed_end0 - elapsed_start0);
  ops_printf("Original time would have been %lf\n", (elapsed_end0 - elapsed_start0) - (elapsed_stride_end - elapsed_stride_start));
  ops_printf("-----------------------------------------\n");
}
