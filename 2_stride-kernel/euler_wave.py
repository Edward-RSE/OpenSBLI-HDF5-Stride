...
# Initial conditions
x0 = parse_expr(
    "Eq(DataObject(x0), block.deltas[0]*block.grid_indexes[0])", local_dict=local_dict
)
x1 = parse_expr(
    "Eq(DataObject(x1), block.deltas[1]*block.grid_indexes[1])", local_dict=local_dict
)


d_in = parse_expr(
    "Eq(GridVariable(d), 1+0.2*sin(pi*(DataObject(x0)+DataObject(x1))))",
...
rhoE = parse_expr(
    "Eq(DataObject(rhoE), p/(gama-1) + 0.5* d *(u0**2+u1**2))", local_dict=local_dict
)
...
kwargs = {"iotype": "Write"}
h5 = iohdf5(save_every=25, **kwargs)
