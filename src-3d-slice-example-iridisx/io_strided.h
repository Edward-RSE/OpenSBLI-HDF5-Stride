 ...
 *
 */
ops_dat create_strided_ops_dat_with_halo_cells_single_precision(ops_dat dat, size_t stride_i, size_t stride_j,
                                                                size_t stride_k) {
  /*
   * Determine the size of the strided dat halos and the actual size of the
   * dataset. This is taken from the original ops_dat, but it could be hardcoded
  ...
  float *dummy = NULL;
  ops_dat strided_dat =
      ops_decl_dat(dat->block, dat->dim, strided_size, dat->base, strided_d_m, strided_d_p, dummy, "float", dat->name);
  ...
  for (int k = dat_start[2]; k < dat_stop[2]; k += stride_k) {
    for (int j = dat_start[1]; j < dat_stop[1]; j += stride_j) {
      for (int i = dat_start[0]; i < dat_stop[0]; i += stride_i) {
        ...
        /*
         * Convert the data from double to float. This is done using memcpy
         * because ops_dat->data is a char* so we are working in bytes, rather
         * than structured data. Then use a static cast to convert to float. I
