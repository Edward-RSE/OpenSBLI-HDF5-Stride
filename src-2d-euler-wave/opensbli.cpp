...
#include "bc_exchanges.h"
  ...
  for (iter = start_iter; iter <= start_iter + niter - 1; iter++) {
    ...
    for (stage = 0; stage <= 2; stage++) {
      ...
      ops_par_loop(opensbliblock00Kernel006, "CRp", opensbliblock00, 2, iteration_range_6_block0,
                   ops_arg_dat(rhoE_B0, 1, stencil_0_00_00_2, "double", OPS_READ),
                   ops_arg_dat(rho_B0, 1, stencil_0_00_00_2, "double", OPS_READ),
  ...
  HDF5_IO_Write_Strided_0_opensbliblock00(opensbliblock00, rho_B0, rhou0_B0, rhou1_B0, rhoE_B0, x0_B0, x1_B0,
                                          HDF5_timing);
  ops_exit();
  // Main program end
}