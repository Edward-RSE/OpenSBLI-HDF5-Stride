...
void restrict_kernel(const ACC<double> &original_dat, ACC<float> &strided_dat, const int *idx) {
  strided_dat(0, 0, 0) = (float)original_dat(0, 0, 0);
}
...
void copy_to_strided_dat(ops_block block, int block0np0, int block0np1, int block0np2, int stride[],
  ...
  ops_par_loop(restrict_kernel, "restrict_kernel", block, 3, iter_range,
               ...
               ops_arg_dat(strided_dat, 1, stencil2d_00, "float", OPS_WRITE), ops_arg_idx());
}
...
void HDF5_IO_Init_0_opensbliblock00_strided(ops_block block, int block0np0, int block0np1, int block0np2,
   ...
   */
  rho_B0_strided = ops_decl_dat(block, 1, strided_size, strided_base, strided_d_m, strided_d_p, stride, dummy, "float",
                                "rho_B0_strided");
