...
# Few hundred million points.... one or two GPUs
...
if weno or teno:
    ...
    momentum = (
        "Eq(Der(rhou_i,t) , -Conservative(rhou_i*u_j + KD(_i,_j)*p,x_j , %s) + Der(tau_i_j,x_j) )"
        % sc1
    )
    energy = (
        "Eq(Der(rhoE,t), - Conservative((p+rhoE)*u_j,x_j, %s) - Der(q_j,x_j) + Der(u_i*tau_i_j ,x_j) )"
        % sc1
    )
    ...
    for i, base in enumerate(base_eqns):
        base_eqns[i] = einstein_eq.expand(
            base, ndim, coordinate_symbol, substitutions, constants
        )
...
CF_energy = einstein_eq.expand(
    CF_forcing_energy, ndim, coordinate_symbol, [], constants
)
...
metriceq.generate_transformations(
    ndim, coordinate_symbol, [(False, False), (True, False), (False, False)], 2
)
...
wall_energy = [
    Eq(q_vector[-1], Twall * q_vector[0] / (gama * Minf**2.0 * (gama - S.One)))
]
...
stretched_eqn = (
    0.5
    * Ly
    * (1.0 - ((tanh(stretch * (1.0 - 2.0 * (j / (ny - 1.0))))) / (tanh(stretch))))
    - 1.0
)
...
initial_equations += [
    Eq(cx, cos(4.0 * pi * x / lx)),
    Eq(cy, S.One + cos(pi * y)),
    Eq(cz, cos(2.0 * pi * z / lz)),
]
...
initial_equations += [
    Eq(q_vector[4], p / (gama - S.One) + 0.5 * d * (u**2 + v**2 + w**2))
]
...
if stats:
    ...
    statistics = [
        DataObject("u2u2mean"),
        DataObject("rhou2mean"),
        DataObject("rhou2u1mean"),
        DataObject("u0u0mean"),
        DataObject("rhou1u0mean"),
        DataObject("E_mean"),
        DataObject("u1u0mean"),
        DataObject("u1u1mean"),
        DataObject("rhou2u0mean"),
        DataObject("rhou0mean"),
        DataObject("rhou1mean"),
        DataObject("pp_mean"),
        DataObject("rhou2u2mean"),
        DataObject("u2mean"),
        DataObject("M_mean"),
        DataObject("u2u0mean"),
        DataObject("p_mean"),
        DataObject("a_mean"),
        DataObject("T_mean"),
        DataObject("rhou0u0mean"),
        DataObject("rhomean"),
        DataObject("mu_mean"),
        DataObject("u2u1mean"),
        DataObject("TT_mean"),
        DataObject("rhou1u1mean"),
        DataObject("u0mean"),
        DataObject("u1mean"),
        DataObject("D11"),
    ]
...
if not weno and not teno and not TVD:
    slice_arrays = [
        DataObject("%s" % i) for i in ["rho", "rhou0", "rhou1", "rhou2", "rhoE"]
    ] + [DataObject("WENO_filter")]
else:
    slice_arrays = [
        DataObject("%s" % i) for i in ["rho", "rhou0", "rhou1", "rhou2", "rhoE"]
    ]
...
if explicit_filters:
    ...
    filter_swaps = create_exchange_calls_codes(
        block, ["rho", "rhou0", "rhou1", "rhou2", "rhoE"]
    )
